...
**Justificación del diseño de los criterios:** C1 es trivial pero obligatorio (es el umbral del negocio). El valor real está en **C2 y C3**, que detectan intentos de **evadir** C1 — que es exactamente lo que un motor de compliance debe buscar. Los umbrales (`N`, ventana, %) se dejan como **parámetros configurables** por variables de entorno, no hardcodeados, porque el "nivel correcto" es una decisión de política de negocio, no de ingeniería (decisión cerrada en `DECISIONS.md`, ADR-005). C4 (vaciado) queda **diferido** con justificación (requiere snapshot de saldo).
